...
// Mocked tag mapping for topics
const TOPICS = ['Array', 'String', 'DP', 'Greedy', 'Tree', 'Graph', 'Linked List', 'Math', 'Stack'];
...
export function QuestionsTable({ data = [] }) {
  ...
  const getPopularityBadge = (freq) => {
    ...
    if (f > 80) return { label: 'VERY HOT', color: 'text-red-500', bg: 'bg-red-500/10' };
    if (f > 60) return { label: 'HOT', color: 'text-orange-500', bg: 'bg-orange-500/10' };
    if (f > 40) return { label: 'WARM', color: 'text-yellow-500', bg: 'bg-yellow-500/10' };
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-800 bg-[#1a1a1a]/50">
                ...
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Popularity</th>
                  ...
                  <tr key={q.id} className="group hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-5 h-5 rounded-full bg-black border border-gray-800 flex items-center justify-center p-1">
                          <img src="https://leetcode.com/static/images/LeetCode_logo_rvs.png" alt="LC" className="w-full h-auto" />
                        </div>
                        <span className="text-xs text-gray-400 font-medium">LeetCode</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={cn(
                    ...
                    <td className="px-6 py-4">
                      <div className={cn(
                        ...
                        <span className={cn("text-[10px] font-bold", pop.color)}>{pop.label}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
